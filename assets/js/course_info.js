...
function renderDetailCourse(index) {
  let removed_element = document.querySelectorAll(
    ".content_main--advantage_group"
  );
  ...
  for (let i = 0; i < listUserPassword.length; i++) {
    for (let j = 0; j < listUserPassword[i].course.length; j++) {
      if (listUserPassword[i].course[j].id == index) {
        ...
        if (
          reviewCourseValue != "" &&
          reviewCourseValue != null &&
          reviewCourseValue != undefined
        ) {
  ...
  //compare all info
  course_info_group =
    `<div class="content_main--detail_course--info_02">` +
    course_info +
    user_option +
    about +
    review +
