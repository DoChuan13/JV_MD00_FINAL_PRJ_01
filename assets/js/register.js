...
function checkForm() {
    ...
    if (checkEmail() && checkPassword() && checkRepassword() && checkFirstName() && checkLastName() && checkMobilePhone()) {
        return true;
    }
    else {
    ...
    if (checkForm()) {
        ...
        if (listUserPassword.length == 0) {
            if (password == re_password) {
                let new_user = new UserInfo(id, first_name, last_name, phone, email, password);
