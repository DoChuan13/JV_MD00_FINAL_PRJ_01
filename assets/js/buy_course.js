...
if (checkLogin() == "user") {
      ...
      if (listUserPassword[i].email == loginStatus[0].email) {
        ...
        if (listUserPassword[i].course.length == 0) {
          ...
          let courseValue = {
            ...
            expire: expire.setDate(
              purchaseDate.getDate() + courseDataList[id].coursetime
            ),
          };
          ...
          for (let j = 0; j < listUserPassword[i].course.length; j++) {
            if (
              listUserPassword[i].course[j].course == buyOderPending[0].course
            ) {
              listUserPassword[i].course[j].renew += 1;
              listUserPassword[i].course[j].expire = expire.setDate(
                purchaseDate.getDate() +
                  courseDataList[id].coursetime *
  ...
  function buyCourse() {
    ...
    let listActiveCourseKey = JSON.parse(
      localStorage.getItem("listActiveCourseKey")
    );
    let listUserPassword = JSON.parse(localStorage.getItem("listUserPassword"));
    let loginStatus = JSON.parse(localStorage.getItem("loginStatus"));
              ...
              if (listActiveCourseKey[i].code[j].value == checkcode.value) {
                if (listActiveCourseKey[i].code[j].status) {
                  ...
                  localStorage.setItem(
                    "listActiveCourseKey",
                    JSON.stringify(listActiveCourseKey)
                  );
                  console.log(
                    "%cMã thẻ hợp hợp lệ, thanh toán thành công",
                    "color: green"
                  );
                  showAlertPopup();
                  let popup_detail = document.querySelector(
                    "#pop_up_alert--detail p"
                  );
                  ...
                  console.log(
                    "%cMã thẻ ĐÃ được sử dụng, vui lòng thử lại mã khác",
                    "color: red"
                  );
                  showAlertPopup();
                  let popup_detail = document.querySelector(
                ...
                console.log(
                  "%cMã thẻ không hợp lệ, vui lòng thử lại mã khác",
                  "color: red"
                );
                showAlertPopup();
                let popup_detail = document.querySelector(
